...
app.get("/health",(req,res)=>{
  try{res.json({ok:true,shop:normalizeShop(requiredEnv("SHOPIFY_STORE")),apiVersion:API_VERSION,time:new Date().toISOString()});}
  catch(e){res.status(500).json({ok:false,error:e.message});}
});
...
app.post("/api/products/import",verifyKey,async(req,res)=>{
  try{
    ...
    res.json({ok:true,action,sku:p.sku,quantity:p.quantity,location:location.name,
